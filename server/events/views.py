...
def index(request):
	...
	"""
    return HttpResponse(
        'Maintenance section of the heltech site, currently: <a href="authorize">Authorizing the server</a> for facebook event access is all you can do. Site administration needs to remove the token, if needed.<br> Before authorization, a Facebook application details need to have inserted via the admin site by site admin.')
...
def authorize(request):
    ...
    try:
        ...
        return HttpResponse(
            'Cannot authorize, a facebook application details need to be created through the admin site.')
